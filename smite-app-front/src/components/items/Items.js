...
class Items extends Component {
  ...
  renderItemList = () => {
    ...
    if (this.state.name !== "") {
      ...
      return filteredItems.map(item => <li key={item.item_id}><Link to={`items/${item.item_id}`}><Media><img width={50} height={50} className="mr-3" src={item.item_image} alt={item.name}/></Media>{item.name}</Link></li>)
    }
